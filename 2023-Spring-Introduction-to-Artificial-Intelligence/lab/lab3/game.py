...
class GameStateData:
    ...
    def __init__(self, prevState=None):
        ...
        if prevState != None:
            ...
            self.predict = prevState.predict
    ...
    def deepCopy(self):
        ...
        '''
        ret =  deepcopy(self)
        ret.layout = self.layout.deepCopy()
        return ret
...
class Game:
    ...
    def __init__(self, agents, display, rules, startingIndex=0, muteAgents=False, catchExceptions=False, stepbound=10000):
    ...
    def run(self, visualize):
        ...
        while not self.gameOver:
            ...
            if self.catchExceptions:
                try:
                    ...
                    try:
                        ...
                        ret = timed_func(observation)
                        if agentIndex == 0:
                            action, self.state.data.layout.predy = ret
                        else:
                            action = ret
                    except TimeoutFunctionException:
                ...
                ret = agent.getAction(observation)
                if agentIndex == 0:
                    action, self.state.data.layout.predy = ret
                else:
                    action = ret
            ...
            if num_step == 1 and visualize == True:
