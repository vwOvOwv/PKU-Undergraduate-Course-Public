...
class PacmanGraphics:
    ...
    def drawStaticObjects(self, state):
        ...
        self.drawFood(layout.food, layout.figureidx, layout.y, layout.predy)
    ...
    def drawFood(self, foodMatrix, figureidx, y, predy):
        try:
            if predy == None:
                for xNum, x in enumerate(foodMatrix):
                    for yNum, cell in enumerate(x):
                        if cell:  # There's food here
                            ...
                            tdata = mnist.getdata(idx)
                            for y,x in np.stack(np.where(tdata>200),axis=-1):
                                if y%(28//DEFAULT_FOOD_SIZE)!=0 and x%(28//DEFAULT_FOOD_SIZE)!=0:
                                    continue
                                c = tdata[y,x]/256
                                digitImages.append(circle((screen_x+x*(DEFAULT_FOOD_SIZE/28)-DEFAULT_FOOD_SIZE//2,screen_y+y*(DEFAULT_FOOD_SIZE/28)-DEFAULT_FOOD_SIZE//2),1e-1000,formatColor(c,c,c),formatColor(c,c,c)))
            ...
            for xNum, x in enumerate(foodMatrix):
                for yNum, cell in enumerate(x):
                    if cell:
                        (screen_x, screen_y) = self.to_screen((xNum, yNum))
                        digitImages=[]
                        digitImages.append(text ((screen_x+4, screen_y+4),
                                           formatColor(.9, 0, 0),predy[xNum, yNum], "Times", 10, "bold"))
                        self.predImages[(xNum,yNum)]=digitImages
