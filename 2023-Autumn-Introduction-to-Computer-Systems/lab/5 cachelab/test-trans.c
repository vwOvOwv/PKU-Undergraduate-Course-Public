...
void eval_perf(unsigned int s, unsigned int E, unsigned int b)
{
  ...
  for (i=0; i<func_counter; i++) {
    ...
    sprintf(cmd, "valgrind --tool=lackey --trace-mem=yes --log-fd=1 -v ./tracegen -M %d -N %d -F %d", M, N,i);
    ...
    while (fgets(buf, 1000, full_trace_fp) != NULL) {
      ...
      if (buf[0]==' ' && buf[2]==' ' &&
        ...
        if ((log_cnt += cnt) > LOG_LIMIT) {
          printf("Too many memory operations (store/load/modify)! Please check your code to remove redundant memory operations OR there is something wrong with your code that memory misses are too large to count!\n");
          flag = 1; break;
        }
        p = p + cnt;
      }
