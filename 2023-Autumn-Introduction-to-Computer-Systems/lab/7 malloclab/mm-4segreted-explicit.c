...
static char *size16_31root = 0;
static char *size32_63root = 0;
static char *size64_127root = 0;
static char *size128_255root = 0;
static char *size256_511root = 0;
static char *size512_1023root = 0;
static char *size1024_2047root = 0;
static char *size2048_4095root = 0;
static char *size4096_8191root = 0;
static char *size8192_infroot = 0;
...
static void _insert_node(const void *bp, char **root);
static void delete_node(const void *bp);
static void _delete_node(const void *bp, char **root);
static void edit_node(const void *bp, size_t csize);
static void _edit_node(const void *bp, char **root, size_t csize);
...
static char **num2list(int index);
static int list2num(const char* root);
static unsigned get_list_size(int index);
...
static void insert_node(const void *bp){
    ...
    if(size>=16&&size<32)
        _insert_node(bp, &size16_31root);
    else if (size>=32&&size<64)
        _insert_node(bp, &size32_63root);
    else if (size>=64&&size<128)
        _insert_node(bp, &size64_127root);
    else if (size>=128&&size<256)
        _insert_node(bp, &size128_255root);
    else if (size>=256&&size<512)
        _insert_node(bp, &size256_511root);
    else if (size>=512&&size<1024)
        _insert_node(bp, &size512_1023root);
        ...
        _insert_node(bp, &size8192_infroot);
}
static void _insert_node(const void *bp, char** root){
    if(*root){
        PUT(*root,REL_DIS(bp));
...
static void delete_node(const void *bp){
    unsigned int size = GET_SIZE(HDRP(bp));
    if(size>=16&&size<32)
        _delete_node(bp, &size16_31root);
    else if (size>=32&&size<64)
        _delete_node(bp, &size32_63root);
    else if (size>=64&&size<128)
        _delete_node(bp, &size64_127root);
    else if (size>=128&&size<256)
        _delete_node(bp, &size128_255root);
    else if (size>=256&&size<512)
        _delete_node(bp, &size256_511root);
    else if (size>=512&&size<1024)
        _delete_node(bp, &size512_1023root);
    else if (size>=1024&&size<2048)
        ...
        _delete_node(bp, &size8192_infroot);
}
static void _delete_node(const void *bp, char** root){
    char *prev,*next;
...
static void edit_node(const void *bp, size_t csize){
    if(csize>=16&&csize<32)
        _edit_node(bp, &size16_31root, csize);
    else if (csize>=32&&csize<64)
        _edit_node(bp, &size32_63root,csize);
    else if (csize>=64&&csize<128)
        _edit_node(bp, &size64_127root,csize);
    else if (csize>=128&&csize<256)
        _edit_node(bp, &size128_255root,csize);
    else if (csize>=256&&csize<512)
        _edit_node(bp, &size256_511root,csize);
    else if (csize>=512&&csize<1024)
        _edit_node(bp, &size512_1023root,csize);
        ...
        _edit_node(bp, &size8192_infroot,csize);
}
static void _edit_node(const void *bp, char** root, size_t csize){//分割出来的块可能在别的大小类
...
static char **num2list(int index){
    switch (index) {
    case 0:
        return &size16_31root;
    case 1:
        return &size32_63root;
    case 2:
        return &size64_127root;
    case 3:
        return &size128_255root;
    case 4:
        return &size256_511root;
    case 5:
        return &size512_1023root;
    case 6:
    ...
    case 9:
        return &size8192_infroot;
    }
    return NULL;
}

static int list2num(const char *root){
    if (root == size16_31root)
        return 0;
    if (root == size32_63root)
        return 1;
    if (root == size64_127root)
        return 2;
    if (root == size128_255root)
        return 3;
    if (root == size256_511root)
        return 4;
    if (root == size512_1023root)
        return 5;
    ...
    if (root == size4096_8191root)
        return 8;
    if (root == size8192_infroot)
        return 9;
    return -1;
}

static unsigned get_list_size(int index){
    if(index>=0&&index<LIST_NUM-1)
        return (1<<(index+5))-1;
