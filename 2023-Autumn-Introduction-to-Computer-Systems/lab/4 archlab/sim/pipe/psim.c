...
static void run_tty_sim() 
{
    ...
    /* Emit CPI statistics */
    {
	double cpi = n_instructions > 0 ? (double) cycles/n_instructions : 1.0;
	printf("CPI: %lld cycles/%lld instructions = %.2f\n",
	       cycles, n_instructions, cpi);
    }

    /* To future TAs:
     *
     * In PartC, the CPE calculation makes uses of `n_instructions`, giving rise to
     * a certain exploit: one can add bubbles to the W stage, causing the `n_instructions++`
     * at line 752 to not execute, resulting in a zero `n_instructions`. This is why sometimes
     * a CPE = 0 appears on the scoreboard.
     * Further, a naive check that filters CPEs that are too low will not work: the student
     * can tuning the W bubble rules, so that it no longer bubbles after some time, and thus
     * get just enough CPE (<= 7.50).
     *  
     * The proposed fix: remember all the addresses of the retired instructions, dump them at
     * the end, and make the driver compare them with the trace of YIS (not implemented yet).
     * Since bubbling W will remove an instruction from the retired list, the exploit will
     * be fixed.
     * 
     * However, due to time pressure, I don't have time to properly implement and test this
     * fix. So it becomes a future-you problem :)
     * 
     * Note: This fix will also patch another issue: that the student can cleverly write a HCL 
     * that behaves differently only on the `ncopy` benchmark. The most easy way to do this is
     * to treat a certain noop at a certain PC as a "key". After reaching a key, the HCL
     * jumps to a different code section, where instructions are reintepreted with arbitrary
     * functionality, enabling new instructions such as "conditional-add-one". All while in
     * the eyes of YIS, the program just goes over the "key" noop and enters a normal code
     * section.
     * Since we are now comparing the entire trace of YIS and HCL, not just the end result,
     * this will not be possible in any way.
     * (Notably, removing this kind of exploit may not be a good thing. The design of this
     * lab does not include adding arbitrary instructions to the ISA; but as a student I
     * would argue that it would be the most fun to do so. By keeping this exploit, we
     * allow nerdy people to really be nerdy!)
     * 
     * XYX 2013.10.18
     */

    // if (verbosity > 0) {
    //     printf("Retired Instruction PC:\n");
...
word_t n_instructions = 0;
/* Collection of retired instructions (thier PC). */
word_t instructions[MAXINSTR];
...
static byte_t sim_step_pipe(word_t max_instr, word_t ccount)
{
	...
	starting_up = 0;
    n_instructions++;
	// instructions[n_instructions++] =  mem_wb_curr->stage_pc;
	cycles++;
    } else {
	if (!starting_up)
	    cycles++;
...
void show_cpi() {
    ...
    sprintf(tcl_msg, "showCPI %lld %lld %.2f",
	    cycles, n_instructions, (double) cpi);
    code = Tcl_Eval(sim_interp, tcl_msg);
    if (code != TCL_OK) {
