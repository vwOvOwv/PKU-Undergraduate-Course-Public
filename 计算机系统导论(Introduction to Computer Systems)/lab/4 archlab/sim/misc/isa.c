...
instr_t instruction_set[] = 
{
    ...
    {"jm",     HPACK(I_JM, F_NONE), 10, I_ARG, 2, 8, R_ARG, 1, 0 },
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_RRMOVQ || hi0 == I_ALU || hi0 == I_PUSHQ ||
	 ...
	 hi0 == I_MRMOVQ || hi0 == I_IADDQ || hi0 == I_JM);
	...
	(hi0 == I_IRMOVQ || hi0 == I_RMMOVQ || hi0 == I_MRMOVQ ||
	 hi0 == I_JMP || hi0 == I_CALL || hi0 == I_IADDQ || hi0 == I_JM);
	...
	case I_JM:
      ...
      argB = get_reg_val(s->r, lo1);
      val = argB + cval;
      if (!get_word_val(s->m, val, &val)) {
        if (error_file)
          fprintf(error_file,
              "PC = 0x%llx, Invalid indirect memory address 0x%llx\n",
              s->pc, val);
        return STAT_ADR;
      }
      s->pc = val;
